...
# 兜底手动 strip
if ! grep -q '^define Build/Compile' $XRAY_MK; then
sed -i '/^define Package\/xray-core\/install/i \
define Build/Compile\
\t$(call GoPackage/Build/Compile)\
\t-$(TARGET_CROSS)strip --strip-all $(PKG_INSTALL_DIR)/usr/bin/*\
endef\
' $XRAY_MK
fi
...
rm -rf feeds/smpackage/luci-theme-design && git clone -b js --single-branch https://github.com/0x676e67/luci-theme-design feeds/smpackage/luci-theme-design
